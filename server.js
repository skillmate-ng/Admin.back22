...
// Visit /api/db-check in a browser to see exactly which tables can be
// read/written and what error (if any) Supabase returns for each.
app.get("/api/db-check", async (_req, res) => {
  ...
  for (const t of tables) {
    ...
    try {
      ...
      if (error) {
        entry.read_error = { code: error.code, message: error.message, hint: error.hint || null };
      } else {
        entry.readable = true;
        entry.row_count = count;
      }
    } catch (e) {
      entry.read_error = { message: e.message };
    }
...
// Visit /api/db-write-test to actually attempt an insert + delete on the
// profiles table, so you can see the *exact* Supabase error a real save
// would hit (e.g. RLS policy denial, missing column, bad constraint).
app.get("/api/db-write-test", async (_req, res) => {
  ...
  res.json({
    ...
    cleanup_ok: !deleteResult.error,
    cleanup_error: deleteResult.error ? deleteResult.error.message : null
  });
});
...
app.get("/api/admin/users/:id", requireAdmin, async (req,res)=>{
  ...
  try{
    ...
    for(const table of tables){
      ...
      else if(table==="requests"||table==="verification_requests"||table==="featured_requests"||table==="referrals"||table==="withdrawals"||table==="activity_log") q=q.eq("user_id",uid);
